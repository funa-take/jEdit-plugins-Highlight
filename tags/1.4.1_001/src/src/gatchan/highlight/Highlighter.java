...
class Highlighter extends TextAreaExtension implements HighlightChangeListener
{
    ...
    int y)
  {
    ...
    {
      highlightList(gfx, screenLine, physicalLine, y);
    }
  }
  
  
  private void highlightList(Graphics2D gfx, int screenLine, int physicalLine, int y)
  {
    ...
    for (int i = 0; i < highlightManager.countHighlights(); i++)
    {
      ...
      highlight(highlight, buffer, screenLine, physicalLine, gfx, y);
      tempLineContent.offset = lineContent.offset;
      tempLineContent.count = lineContent.count;
    }
    highlight(HighlightManagerTableModel.currentWordHighlight, buffer, screenLine, physicalLine, gfx, y);
  }
    ...
    int y)
  {
    ...
    {
      highlight(highlight, screenLine, physicalLine, gfx, y);
    }
  }
    ...
    int y)
  {
		...
		try
    {
      ...
      while (true)
      {
        ...
        int startScreenLine = textArea.getScreenLineOfOffset(textArea.getLineStartOffset(physicalLine) + match.start + i);
        int endScreenLine = textArea.getScreenLineOfOffset(textArea.getLineStartOffset(physicalLine) + match.end + i);
        // System.out.println("startScreenLine = " + startScreenLine );
        // System.out.println("endScreenLine = " + endScreenLine );
        // System.out.println("ScreenLine = " + screenLine );
        if (screenLine >= startScreenLine && screenLine <= endScreenLine) { 
          _highlight(highlight.getColor(), match.start + i, match.end + i, screenLine, physicalLine, gfx, y);
        }
    ...
    int y)
  {
    Point p = null;
    try {
      p = textArea.offsetToXY(physicalLine, startOffset, point);
    } catch (Exception e){
    ...
    int lineStartOffset = textArea.getLineStartOffset(physicalLine);
    boolean isStartLine = screenLine == textArea.getScreenLineOfOffset(lineStartOffset + startOffset);
    boolean isEndLine = screenLine == textArea.getScreenLineOfOffset(lineStartOffset + endOffset);
    if (!isStartLine || !isEndLine){
      if (isStartLine) {
        endX = textArea.getWidth();
      } else if (isEndLine) {
        startX = 0;
      } else {
        endX = textArea.getWidth();
